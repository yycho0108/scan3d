...
def from_h(lmk):
    print lmk.shape
    return lmk[0:3, 0] / lmk[3, 0]

def project(txn, rxn, lmk, K):
    #pt3 = rotate(rxn, from_h(lmk)) + txn
    pt3 = rotate_axa(rxn, from_h(lmk)) + txn
...
def main():
    ...
    for ex, v in rdc1:
        s = '{}={}'.format(ex, v)
        print s.replace('cos', 'np.cos').replace('sin', 'np.sin').replace('sqrt','np.sqrt')
    cex1 = cex1[0]

    names = 'point', 'j_txn', 'j_rxn', 'j_lmk'
    exs = [cex1[:, 0], cex1[:, 1:4], cex1[:, 4:7], cex1[:, 7:11]]
